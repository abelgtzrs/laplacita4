...
export default function ProductsPage() {
  ...
  const categories = [
    ...
    { value: "meats", label: t("category.meats") },
    { value: "groceries", label: t("category.groceries") },
    { value: "produce", label: t("category.produce") },
    { value: "drinks", label: t("category.drinks") },
    { value: "dairy", label: t("category.dairy") },
    { value: "bakery", label: t("category.bakery") },
  ]
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== "all") {
      filtered = filtered.filter(
        (product: any) =>
          product.category_en.toLowerCase().includes(selectedCategory) ||
          product.category_es.toLowerCase().includes(selectedCategory),
      )
    }
    ...
    switch (sortBy) {
      case "price_low":
        filtered.sort((a: any, b: any) => Number.parseFloat(a.price) - Number.parseFloat(b.price))
        break
      case "price_high":
        filtered.sort((a: any, b: any) => Number.parseFloat(b.price) - Number.parseFloat(a.price))
        break
      case "alphabetical":
            ...
            filteredProducts.map((product: any) => (
              <Card key={product.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold text-green-600">${Number.parseFloat(product.price).toFixed(2)}</p>
                </CardContent>
              </Card>
            ))
          ) : (
